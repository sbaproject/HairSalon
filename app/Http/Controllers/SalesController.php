...
{ 
    ...
    public function getSalesNew() {
        ...
        return view('pages.sales_new',compact('list_course','list_sales_count','list_customer','list_staff','list_option'));
    }   
    ...
    public function getSalesEdit($id,$index) {
        ...
        return view('pages.sales_edit', compact('sales', 'list_course','index','list_customer','list_staff'));
    }
    ...
    public function getCustomerFirstNameAjax(Request $request) {        
        $outpub = '';        
        if($request->get('c_id'))
        {
            $customer = Customer::where('c_id',$request->get('c_id'))->first();     
            $outpub .= $customer->c_firstname;
       }       
       echo $outpub  ;
    }

    public function getCustomerLastNameAjax(Request $request) {        
        $outpub = '';        
        if($request->get('c_id'))
        {
            $customer = Customer::where('c_id',$request->get('c_id'))->first();        
            $outpub .= $customer->c_lastname;       
       }       
       echo $outpub;
    }

    public function getCourseOption1Ajax(Request $request) {        
        $outpub = '';        
        if($request->get('co_id'))
        {
            $course = Course::where('co_id',$request->get('co_id'))->where('co_del_flg', 0)->first();        
            $outpub .= $course->Option1->op_name;       
       }       
       echo $outpub;
    }

    public function getCourseOption2Ajax(Request $request) {        
        $outpub = '';        
        if($request->get('co_id'))
        {
            $course = Course::where('co_id',$request->get('co_id'))->where('co_del_flg', 0)->first();        
            $outpub .= $course->Option2->op_name;
       }       
       echo $outpub;
    }

    public function getCourseOption3Ajax(Request $request) {        
        $outpub = '';        
        if($request->get('co_id'))
        {
            $course = Course::where('co_id',$request->get('co_id'))->where('co_del_flg', 0)->first();       
            $outpub .= $course->Option3->op_name;        
       }       
       echo $outpub;
    }

    public function getCourseMoneyAjax(Request $request) {        
        $outpub = '';        
        if($request->get('co_id'))
        {
            $course = Course::where('co_id',$request->get('co_id'))->where('co_del_flg', 0)->first();        
